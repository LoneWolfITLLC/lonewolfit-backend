...
const db = new sqlite3.Database("./database.sqlite", (err) => {
  if (err) {
    ...
    db.run(
      ...
      (err) => {
        if (err) {
          console.error("Error creating the bookings table:", err.message);
...
app.post("/api/cleanup-past-bookings", authenticateJWT, async (req, res) => {
  const userId = req.user.id;
  ...
  try {
    await cleanupPastBookings(admin);
    return res
      .status(200)
    ...
    return res.status(500).send({
      error:
        "An error occurred during the cleanup of the past bookings: " + error,
    });
  }
});
app.post(
  "/api/cleanup-past-consultations",
  authenticateJWT,
  async (req, res) => {
    const userId = req.user.id;
    ...
    try {
      await cleanupPastConsultations(admin);
      return res
        .status(200)
        .send({ message: "Cleanup completed successfully..." });
    } catch (error) {
      ...
      return res.status(500).send({
        error:
          "An error occurred during the cleanup of the past consultations: " +
          error,
      });
    }
  }
);
async function cleanupPastBookings(admin) {
  if (!admin) return;

  const currentTime = Date.now(); // Get the current timestamp

  // SQL query to delete past bookings; only those whose end time is before the current time
  db.run(
    `DELETE FROM bookings 
         WHERE requested_time < ? AND (requested_time + length) < ?`,
    [currentTime, currentTime],
    function (err) {
      if (err) {
        console.error("Error deleting past bookings:", err.message);
      } else {
        console.log(`${this.changes} booking(s) deleted.`);
      }
    }
  );
}
async function cleanupPastConsultations(admin) {
  if (!admin) return;
  ...
  db.run(
    ...
    function (err) {
      if (err) {
        console.error("Error deleting past consultations:", err.message);
      } else {
        console.log(`${this.changes} consultation(s) deleted.`);
      }
    }
  );
}
...
async function cleanupDeletedUserContent() {
  ...
  db.all(`SELECT id FROM users`, [], (err, rows) => {
    ...
    // Delete consultations for non-existing users
    db.run(
      `DELETE FROM consultations WHERE user_id NOT IN (${Array.from(
        existingUserIds
      ).join(",")})`,
      (err) => {
        if (err) {
          console.error(
            "Error deleting consultations for deleted users:",
            err.message
          );
        } else {
          console.log("Deleted consultations for users that no longer exist.");
        }
      }
    );
    // Delete bookings for non-existing users
    db.run(
      `DELETE FROM bookings WHERE user_id NOT IN (${Array.from(
        existingUserIds
      ).join(",")})`,
      (err) => {
        if (err) {
          ...
          console.log("Deleted bookings for users that no longer exist.");
        }
      }
    );
  });
  ...
  try {
    const consultations = await new Promise((resolve, reject) => {
      db.all(`SELECT * FROM consultations`, [], (err, rows) => {
        if (err) {
          console.error("Error retrieving consultations:", err.message);
          return reject(err);
        }
        resolve(rows);
      });
    });
    // Send the testimonials as a JSON response
    return res.status(200).json(consultations);
  } catch (error) {
    return res
      .status(500)
      .send({ error: "An error occurred while retrieving consultations." });
  }
});
app.get("/api/user/consultations", authenticateJWT, async (req, res) => {
  const userId = req.user.id;
  // Retrieve all consultations for the user here
  try {
    const consultations = await getConsultations(userId, true);
    // Send the testimonials as a JSON response
    return res.status(200).json(consultations);
  } catch (error) {
    return res.status(500).send({
      error:
        "An error occurred while retrieving consultations for the specified user.",
    });
  }
});
async function getConsultations(userId, sortByRequestedTime) {
  try {
    return await new Promise((resolve, reject) => {
      const query = sortByRequestedTime
        ? `SELECT * FROM consultations WHERE user_id = ? ORDER BY requested_time`
        : `SELECT * FROM consultations WHERE user_id = ? ORDER BY time`; // Get the latest consultation for the user
      db.all(query, [userId], (err, rows) => {
        if (err) {
          console.error("Error retrieving consultations:", err.message);
          return reject(err);
        }
        resolve(rows); // Resolve with the rows retrieved from the database
      });
    });
  } catch (error) {
    console.error("Error in getConsultations:", error);
    throw new Error("Failed to retrieve consultations"); // Optionally rethrow with a custom error message
  }
}
async function getBookings(userId, sortByRequestedTime) {
  try {
    return await new Promise((resolve, reject) => {
      const query = sortByRequestedTime
        ? `SELECT * FROM bookings WHERE user_id = ? ORDER BY requested_time`
        : `SELECT * FROM bookings WHERE user_id = ? ORDER BY time`; // Get the latest consultation for the user
      db.all(query, [userId], (err, rows) => {
        if (err) {
    ...
    throw new Error("Failed to retrieve bookings"); // Optionally rethrow with a custom error message
  }
}
app.get("/api/bookings", authenticateJWT, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .send({ error: "An error occurred while retrieving bookings." });
  }
});
app.get("/api/user/bookings", authenticateJWT, async (req, res) => {
  ...
  // Retrieve all consultations for the user here
  try {
    const bookings = await getBookings(userId, true);
    // Send the testimonials as a JSON response
    return res.status(200).json(bookings);
  } catch (error) {
    return res.status(500).send({
      error:
        "An error occurred while retrieving bookings for the specified user.",
    });
  }
});
app.post("/api/submit-consultation", authenticateJWT, async (req, res) => {
  const { note, requested_time, phone } = req.body; // Extract note and requested_time from the request body
  const user_id = req.user.id; // Extract user ID from JWT
  const phoneNum = phone;

  // Log the request
  console.log("User requested a consultation");

  // Validate that the required fields are present
  if (!requested_time || !note) {
    return res
      .status(400)
      .send({ error: "Requested time and note are required." });
  }

  // Get the current timestamp for the consultation
  const time = Date.now(); // Current time in milliseconds

  if (requested_time < time + 1000) {
    return res.status(500).send({
      error:
        "Requested time cannot be before or the same as the current time...",
    });
  }
  ...
  if (note.length > 250) {
    return res.status(500).send({
      error:
        "Consultation note cannot be more than 250 characters in length...",
    });
  }

  if (Number.isNaN(phoneNum) || phoneNum === null) {
    return res.status(400).send({
      error:
        "Requested callback number is not a numerical value or does not exist...",
    });
  }

  // Fetch the user's last consultation submission
  try {
    const lastConsultation = await getConsultations(user_id, false);

    // Check if the user has submitted a consultation within the last 5 minutes
    const FIVE_MINUTES = 5 * 60 * 1000; // 5 minutes in milliseconds
    const lastSubmissionTime =
      lastConsultation.length > 0 ? lastConsultation[0].time : null;

    if (lastSubmissionTime && time - lastSubmissionTime < FIVE_MINUTES) {
      return res
        .status(429)
        .send({ error: "You can only submit a consultation every 5 minutes." });
    }
  } catch (error) {
    console.error("Error checking last consultation:", error);
    return res.status(500).send({ error: "Internal Server Error" });
  }

  // Insert the consultation into the consultations table
  const query = `INSERT INTO consultations (user_id, time, requested_time, note, phone) VALUES (?, ?, ?, ?, ?)`;

  db.run(
    query,
    [user_id, time, requested_time, note, phoneNum],
    function (err) {
      if (err) {
        console.error("Error inserting consultation:", err.message);
        return res.status(500).send({
          error: "An error occurred while submitting the consultation.",
        });
      }

      // Respond with a success message and the ID of the new consultation
      console.log(
        `User ID: ${user_id} has submitted a consultation ID: ${this.lastID}`
      );
...
app.post("/api/submit-booking", authenticateJWT, async (req, res) => {
  const { requested_time, length, travel_time, note, phone } = req.body;
  ...
  if (
    ...
    return res.status(400).send({
      error: "Requested time, note, length and travel time are required.",
    });
  }
  ...
  if (requested_time < time + 1000) {
    return res.status(400).send({
      error: "Requested time cannot be before or the same as the current time.",
    });
  }

  if (note.length > 250) {
    return res.status(400).send({
      error: "Consultation note cannot be more than 250 characters in length.",
    });
  }
  ...
  if (!/^\d+$/.test(phone)) {
    return res
      .status(400)
      .send({ error: "Requested callback number must be a numerical value." });
  }

  // Calculate booking window
  const bookingStartTime = requested_time - travel_time; // Start time considering travel
  const bookingEndTime = requested_time + length; // End time considering length

  try {
    // Fetch existing bookings for the user
    const existingBookings = await getConflictingBookings(
      user_id,
      bookingStartTime,
      bookingEndTime
    );
    ...
    if (existingBookings.length > 0) {
      console.error("Conflicting with other bookings...");
      return res
        .status(409)
        .send({ error: "There is a conflict with an existing booking." });
    }

    // Insert the booking into the bookings table
    const query = `INSERT INTO bookings (user_id, time, requested_time, length, travel_time, note, phone) VALUES (?, ?, ?, ?, ?, ?, ?)`;

    db.run(
      query,
      [user_id, time, requested_time, length, travel_time, note, phone],
      function (err) {
        if (err) {
          console.error("Error inserting booking:", err.message);
          return res
            .status(500)
            .send({ error: "An error occurred while submitting the booking." });
        }

        // Respond with a success message and the ID of the new booking
        console.log(
          `User ID: ${user_id} has submitted a booking ID: ${this.lastID}`
        );
        return res.status(201).send({
...
// Function to get conflicting bookings
async function getConflictingBookings(user_id, startTime, endTime) {
  return new Promise((resolve, reject) => {
    const query = `SELECT * FROM bookings WHERE user_id = ? AND 
                       ((requested_time BETWEEN ? AND ?) OR 
                       (requested_time + length BETWEEN ? AND ?))`;

    db.all(
      query,
      [user_id, startTime, endTime, startTime, endTime],
      (err, rows) => {
        if (err) {
          return reject(err);
        }
...
app.put("/api/edit-consultation", authenticateJWT, (req, res) => {
  const { consultationId, note, requested_time, phone } = req.body; // Extract note and requested_time from the request body
  const user_id = req.user.id; // Extract user ID from JWT
  const phoneNum = phone;

  // Log the request
  console.log("Administrator requested a consultation edit...");

  const admin = checkAdminStatusByID(user_id);
  if (!admin) {
    return res
      .status(400)
      .send({ error: "USER IS NOT AN APPROVED ADMIN OF LONE WOLF IT..." });
  ...
  // Validate that the required fields are present
  if (!requested_time || !note || !consultationId) {
    return res.status(400).send({
      error: "Requested time, consultation ID, and note are required.",
    });
  }
  ...
  // SQL query to update the consultation by consultationId, allowing admin edits without user_id restriction
  const query = `UPDATE consultations SET requested_time = ?, note = ?, phone = ? WHERE id = ?`;

  // Execute the query
  db.run(
    query,
    [requested_time, note, phoneNum, consultationId],
    function (err) {
      if (err) {
      ...
      if (this.changes === 0) {
        // This indicates that the consultation was not found
        return res.status(404).send({ error: "Consultation not found." });
      }

      // Successfully updated the consultation
      console.log(
        `User ID: ${user_id} has edited a consultation ID: ${consultationId}`
      );
      return res
        .status(200)
        .send({ message: "Consultation edited successfully." });
    }
  );
});
app.put("/api/edit-booking", authenticateJWT, async (req, res) => {
  const { bookingId, note, requested_time, phone, length, travel_time } =
    req.body; // Extract necessary fields
  const user_id = req.user.id; // Extract user ID from JWT

  // Log the request
  console.log("Administrator requested a booking edit...");

  const admin = await checkAdminStatusByID(user_id);
  if (!admin) {
    return res
      .status(400)
      .send({ error: "USER IS NOT AN APPROVED ADMIN OF LONE WOLF IT..." });
  ...
  if (
    ...
    return res.status(400).send({
      error:
        "Requested time, booking ID, note, length, and travel time are required, and length must be positive.",
    });
  }
  ...
  try {
    ...
    // Check if the only conflicting booking is the one being modified.
    if (conflictingBookings.length > 0) {
      // Check if the conflicting booking has the same ID as the one being edited
      const isOnlyConflict = conflictingBookings.every(
    ...
    // SQL query to update the booking by bookingId
    const query = `UPDATE bookings SET requested_time = ?, length = ?, travel_time = ?, note = ?, phone = ? WHERE id = ?`;

    // Execute the SQL update query
    db.run(
      query,
      [requested_time, length, travel_time, note, phone, bookingId],
      function (err) {
        if (err) {
          console.error("Error updating booking:", err.message);
          return res
            .status(500)
            .send({ error: "An error occurred while updating the booking." });
        }

        // Check whether any rows were updated
        if (this.changes === 0) {
          // This indicates that the booking was not found
          return res.status(404).send({ error: "Booking not found." });
        }

        // Successfully updated the booking
        console.log(
          `User ID: ${user_id} has edited a booking ID: ${bookingId}`
        );
        return res
          .status(200)
          .send({ message: "Booking edited successfully." });
      }
    );
  } catch (error) {
    console.error("Error processing booking edit:", error);
    return res.status(500).send({ error: "Internal Server Error" });
  }
});
app.put("/api/user/edit-booking", authenticateJWT, async (req, res) => {
  const { bookingId, note, requested_time, phone, length, travel_time } =
    req.body; // Extract necessary fields
  const user_id = req.user.id; // Extract user ID from JWT
  console.log(
    `PUT /api/user/edit-booking invoked by user ID: ${user_id} with booking ID: ${bookingId}`
  );
  ...
  if (
    ...
    console.log("Validation error: Missing fields or invalid input.");
    return res.status(400).send({
      error:
        "Requested time, booking ID, note, length, and travel time are required, and length must be positive.",
    });
  ...
  if (requested_time < time + 1000) {
    console.log("Validation error: Requested time is before the current time.");
    return res.status(400).send({
      error:
        "Requested time cannot be before or the same as the current time...",
    });
  ...
  if (note.length > 250) {
    console.log("Validation error: Booking note exceeds 250 characters.");
    return res.status(400).send({
      error: "Booking note cannot be more than 250 characters in length...",
    });
  }

  if (!/^\d+$/.test(phone)) {
    console.log(
      "Validation error: Requested callback number is not numerical."
    );
    return res
      .status(400)
      .send({ error: "Requested callback number must be a numerical value." });
  ...
  try {
    ...
    const bookingQuery = `SELECT user_id FROM bookings WHERE id = ?`;
    const bookingRow = await new Promise((resolve, reject) => {
      db.get(bookingQuery, [bookingId], (err, row) => {
        if (err) {
          console.error("Database error when fetching booking:", err);
          reject(err);
        } else {
          resolve(row);
        }
    ...
    if (!bookingRow) {
      console.log(`Booking not found for booking ID: ${bookingId}.`);
      return res.status(404).send({ error: "Booking not found." });
    }

    if (bookingRow.user_id !== user_id) {
      console.log(
        `User ID: ${user_id} attempted to edit booking ID: ${bookingId} without permissions.`
      );
      return res
        .status(403)
        .send({ error: "You do not have permission to edit this booking." });
    }
    ...
    if (conflictingBookings.length > 0) {
      ...
      if (!isOnlyConflict) {
        console.log(
          `Conflict detected for user ID: ${user_id} with booking ID: ${bookingId}.`
        );
        return res
          .status(409)
          .send({ error: "There is a conflict with an existing booking." });
    ...
    // Step 3: SQL query to update the booking by bookingId
    const query = `UPDATE bookings SET requested_time = ?, length = ?, travel_time = ?, note = ?, phone = ? WHERE id = ?`;
    ...
    db.run(
      ...
      function (err) {
        ...
        if (this.changes === 0) {
          console.log(
            `No updates made for booking ID: ${bookingId} (not found or no change).`
          );
          return res.status(404).send({ error: "Booking not found." });
        }

        console.log(
          `User ID: ${user_id} successfully edited booking ID: ${bookingId}.`
        );
        return res
          .status(200)
          .send({ message: "Booking edited successfully." });
...
app.delete("/api/delete-consultation", authenticateJWT, (req, res) => {
  const { consultationId } = req.body; // Expect testimonialId in the request body
  const user_id = req.user.id; // Extract user ID from JWT

  console.log(
    `User ID: ${user_id} is attempting to delete consultation ID: ${consultationId}`
  );
  const admin = checkAdminStatusByID(user_id);
  if (!admin) {
    return res
  ...
  // Check if the consultation exists and delete it
  ...
  db.run(query, [consultationId], function (err) {
    if (err) {
      ...
      return res
        .status(500)
        .send({ error: "An error occurred while deleting the consultation." });
    }
    // Check if any row was deleted
    if (this.changes === 0) {
      // No rows were deleted (consultation might not exist)
      return res.status(404).send({ error: "Consultation not found." });
    }
    console.log(
      `User ID: ${user_id} has deleted a consultation ID: ${consultationId}`
    );
    // Successfully deleted the consultation
    return res
      .status(200)
      .send({ message: "Consultation deleted successfully." });
  });
});
// DELETE '/api/user/delete-consultation'
app.delete(
  "/api/user/delete-consultation",
  authenticateJWT,
  async (req, res) => {
    const { consultationId } = req.body; // Expect consultationId in the request body
    const user_id = req.user.id; // Extract user ID from JWT
    ...
    // Check if the consultation exists and if the user is authorized to delete it
    const getConsultationQuery = `SELECT user_id FROM consultations WHERE id = ?`;

    try {
      const consultation = await new Promise((resolve, reject) => {
        db.get(getConsultationQuery, [consultationId], (err, row) => {
          if (err) {
            return reject(err);
          }
          resolve(row);
      ...
      // Check if the user_id matches the one associated with the consultation
      ...
      // Proceed to delete the consultation
      const deleteQuery = `DELETE FROM consultations WHERE id = ?`;

      db.run(deleteQuery, [consultationId], function (err) {
        if (err) {
          console.error("Error deleting consultation:", err.message);
          return res.status(500).send({
      ...
      console.error("Error processing consultation deletion:", error);
      return res.status(500).send({ error: "Internal Server Error" });
    }
  }
);
// DELETE '/api/delete-testimonial'
app.delete("/api/delete-booking", authenticateJWT, (req, res) => {
  const { bookingId } = req.body; // Expect testimonialId in the request body
  const user_id = req.user.id; // Extract user ID from JWT

  console.log(
    `User ID: ${user_id} is attempting to delete booking ID: ${bookingId}`
  );
  const admin = checkAdminStatusByID(user_id);
  if (!admin) {
    return res
  ...
  db.run(query, [bookingId], function (err) {
    ...
    console.log(`User ID: ${user_id} has deleted a booking ID: ${bookingId}`);
    // Successfully deleted the consultation
    return res.status(200).send({ message: "Booking deleted successfully." });
  });
});
// DELETE '/api/user/delete-consultation'
app.delete("/api/user/delete-booking", authenticateJWT, async (req, res) => {
  const { bookingId } = req.body; // Expect consultationId in the request body
  const user_id = req.user.id; // Extract user ID from JWT
  ...
  // Check if the consultation exists and if the user is authorized to delete it
  const getBookingQuery = `SELECT user_id FROM bookings WHERE id = ?`;

  try {
    const booking = await new Promise((resolve, reject) => {
      db.get(getBookingQuery, [bookingId], (err, row) => {
        if (err) {
          return reject(err);
        }
        resolve(row);
    ...
    if (booking.user_id !== user_id) {
      return res
        .status(403)
        .send({ error: "You are not authorized to delete this booking." });
    }

    // Proceed to delete the consultation
    const deleteQuery = `DELETE FROM bookings WHERE id = ?`;

    db.run(deleteQuery, [bookingId], function (err) {
      if (err) {
        console.error("Error deleting booking:", err.message);
        return res
          .status(500)
      ...
      return res
        .status(200)
        .send({ message: "Consultation booking successfully." });
    });
  } catch (error) {
